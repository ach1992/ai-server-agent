...
CI validates syntax for installer, updater, uninstaller, management code, release builder and security test scripts with `bash -n`.
...
2. PR-head CI and High Assurance Security must be green for the exact candidate;
3. after integration, **main-push** CI and High Assurance Security must be green for the exact release SHA;
4. the release workflow must promote the exact CI-produced artifact for that SHA;
5. the operator repository-setting gates must be verified immediately before release dispatch;
6. post-publication checks must verify immutable release state, exact tag SHA and release/asset attestations.
