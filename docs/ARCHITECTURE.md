...
Origin Rule and Configuration Rule discovery is intentionally phase-specific. The manager reads the exact zone phase entrypoint for `http_request_origin` or `http_config_settings`; it does not enumerate the zone's broad Rulesets collection and infer absence from pagination. An authoritative 404 means the phase entrypoint is absent. Cloudflare can retain an existing phase Ruleset after its last Rule is removed and represent that empty state by omitting `rules` from the phase read or returning `rules: null` from the exact Ruleset read. When the phase read reports that state, the manager re-reads the exact Ruleset ID and normalizes it to an empty Rule array only after the Ruleset ID, zone kind, and requested phase are validated. Other provider/transport errors, contradictory phase/exact state, malformed entrypoints, malformed Rule arrays, duplicate Rule IDs, or ownership/identity drift fail closed before any absence-based CREATE or destructive recovery decision.
