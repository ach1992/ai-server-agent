...
def function(text, name, next_name, body):
...
s = once(
    ...
    "entrypoint unique rule ids",
)

s = function(s, "cf_get_rule", "cf_get_origin_cert", r'''cf_get_rule(){
...
s = function(s, "cf_find_rule_by_marker", "cf_recover_pending_write", r'''cf_find_rule_by_marker(){
...
old_local = '  local phase_entry rc ruleset_id rule_ref ref_match rule_body create_body res ruleset marker pending_rule_body ruleset_desc verify_id rule current_fingerprint pending_fingerprint\n'
new_local = '  local phase_entry rc ruleset_id rule_ref ref_match id_count rule_body create_body res ruleset marker pending_rule_body ruleset_desc verify_id rule current_fingerprint pending_fingerprint\n'
if s.count(old_local) != 2:
    raise SystemExit(f"reconcile locals: expected two matches, found {s.count(old_local)}")
s = s.replace(old_local, new_local, 2)

recorded_old = '''    if [ -n "$owned_rule" ] && [ "$owned_ruleset" = "$ruleset_id" ]; then
      rule="$(jq -c --arg id "$owned_rule" --arg ref "$rule_ref" '.result.rules[]? | select(.id==$id and .ref==$ref)' <<<"$ruleset" | head -n1)"
'''
if s.count(recorded_old) != 2:
    raise SystemExit(f"recorded rule reconciliation: expected two matches, found {s.count(recorded_old)}")
origin_new = '''    if [ -n "$owned_rule" ]; then
...
'''
config_new = '''    if [ -n "$owned_rule" ]; then
      ...
      rule="$(jq -c --arg id "$owned_rule" '.result.rules[] | select(.id==$id)' <<<"$ruleset")"
'''
s = s.replace(recorded_old, origin_new, 1)
s = s.replace(recorded_old, config_new, 1)
p.write_text(s)

p = Path("tests/cloudflare_rulesets_pagination.sh")
s = p.read_text()
s = once(s, 'fail(){ echo "FAIL: $*" >&2; exit 1; }\n', 'fail(){ echo "FAIL: $*" >&2; exit 1; }\nORIG_CF_GET_OPTIONAL="$(declare -f cf_get_optional)"\n', "save optional getter")
anchor = "curl(){ printf '%s\\n403' '{\"success\":false,\"errors\":[{\"code\":9109,\"message\":\"permission denied\"}]}' ; }\n"
insert = r'''# Exact Ruleset reads used by cleanup/recovery must distinguish malformed
...
'''
s = once(s, anchor, insert + anchor, "exact ruleset read tests")
p.write_text(s)
...
# Durable pre-POST journal contains the exact semantic fingerprint before POST.
'''
s = once(s, restore_anchor, restore_insert, "pending marker drift test")
...
# Same-host canonical owned SSL rule is a no-op.
'''
s = once(s, reconcile_anchor, reconcile_insert, "recorded id ref drift tests")
p.write_text(s)
