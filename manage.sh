...
cf_list_zone_rulesets(){
  ...
  local seen_cursor_hashes='|' cursor_hash
  while :; do
    ...
    [ -n "$next_cursor" ] || break
    cursor_hash="$(printf '%s' "$next_cursor" | sha256sum | awk '{print $1}')"
    ...
    seen_cursor_hashes="${seen_cursor_hashes}${cursor_hash}|"
    cursor="$next_cursor"
  done
